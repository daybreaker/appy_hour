...
RSpec.describe "Admin deal management", type: :request do
  ...
  describe "item deals" do
    it "creates an item deal with a name and description" do
      expect {
        post admin_happy_hour_item_deals_path(happy_hour),
          params: { happy_hour_item: { name: "Nachos", category: "food", happy_hour_price: 6, description: "Loaded" } },
          as: :turbo_stream
    ...
    it "requires a description" do
      post admin_happy_hour_item_deals_path(happy_hour),
        params: { happy_hour_item: { name: "Nachos", happy_hour_price: 6, description: "" } },
        as: :turbo_stream
      expect(response).to have_http_status(:unprocessable_content)
      expect(happy_hour.happy_hour_items.count).to eq(0)
    end
  end

  describe "bogo deals" do
    it "creates a bogo deal with a name and description" do
      expect {
        post admin_happy_hour_bogo_deals_path(happy_hour),
          params: { happy_hour_bogo: { buy_quantity: 1, get_quantity: 1, get_discount_type: "free",
                                       applies_to: "wings", item_name: "Boneless Wings", description: "BOGO free" } },
          as: :turbo_stream
